...
def get_db_connection():
    """Crear conexión directa a PostgreSQL con manejo de encoding"""
    ...
    # Forzar UTF-8 inmediatamente después de conectar
    cur = conn.cursor()
...
def safe_decode(value):
    """Decodificar texto de forma segura, reemplazando caracteres inválidos"""
    if value is None:
        return None
    if isinstance(value, bytes):
        # Intentar UTF-8, si falla usar Latin-1, si no, reemplazar
        try:
...
# ============================================
# Endpoint: Persona por ID (SIMPLE)
# ============================================
...
def obtener_persona(persona_id: int):
    ...
    conn = None
    try:
        logger.info(f"Buscando persona id={persona_id}")
        ...
        nombre = safe_decode(row['nombre'])
        apellido = safe_decode(row['apellido'])
        
        logger.info(f"Encontrado: {nombre} {apellido}")
        ...
        if conn:
            conn.close()
        raise HTTPException(status_code=500, detail=f"Error al obtener persona: {str(e)}")

# ============================================
# Endpoint: ListadoRendicion (COMPLEJO)
# ============================================
@router.get("/listado-rendicion/{id_juego}", response_model=List[RendicionItem])
def listado_rendicion(id_juego: int):
    """Reporte de rendición de operaciones"""
    conn = None
    try:
        logger.info(f"Ejecutando reporte id_juego={id_juego}")
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        ...
        cur.execute("""
            ...
            WHERE ope.id_juego = %s AND ope.id_estado = 437 
        ...
        reportes = []
        for row in rows:
            # Decodificación segura de TODOS los campos de texto
            distribuidor = safe_decode(row['distribuidor'])
            
            reporte_item = {
                "numero_operacion": row['numero_operacion'],
                "distribuidor": distribuidor if distribuidor else "",
                "rendido": row['rendido'],
